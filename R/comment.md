...
file type plugins are immediately sourced. Consequently, some Vim-R variables,
such as `R_enable_comment`, will be used at their default values even if their
values have been set in the `init.vim`. The workaround is do not include the
superfluous command `syntax enable` in the `init.vim`. For details, please,
access: <https://github.com/jalvesaq/Vim-R/issues/668>.
