...
if !exists("g:did_vimr_rhelp_functions")
    g:did_vimr_rhelp_functions = 1

    def g:RhelpIsInRCode(vrb: number): number
        var lastsec = search('^\\[a-z][a-z]*{', "bncW")
        var secname = getline(lastsec)
        if line(".") > lastsec && (secname =~ '^\\usage{' || secname =~ '^\\examples{' || secname =~ '^\\dontshow{' || secname =~ '^\\dontrun{' || secname =~ '^\\donttest{' || secname =~ '^\\testonly{')
            return 1
        else
            if vrb
                g:RWarningMsg("Not inside an R section.")
            endif
            return 0
        endif
    enddef

    def g:RhelpComplete(findstart: number, base: string): any
        if findstart
            var line = getline('.')
            var start = col('.') - 1
            ...
            var hwords = ['\Alpha', '\Beta', '\Chi', '\Delta', '\Epsilon',
